...
struct UNITTEST_N(cFloat) : public cUnitTest {
    UNITTEST_METHOD(cFloat) {
        ...
        cFloat32 f2;
        f2._v.u_f = 65536;
        UNITTEST_TRUE(f1._v.u_f == f2._v.u_f);

        f1.put_Bits(cFloat32::k_uOne);
        UNITTEST_TRUE(1.0f == f1._v.u_f);
        f2.put_Bits(cFloat32::k_uTwo);
        UNITTEST_TRUE(2.0f == f2._v.u_f);

        cFloat64 d1(65536);
        cFloat64 d2;
        d2._v.u_d = 65536;
        UNITTEST_TRUE(d1._v.u_d == d2._v.u_d);

        cFloat64 dx1(cTypeLimit<double>::Max());
        UNITTEST_TRUE(dx1._v.u_d > 10);
        cFloat64 dx2(cTypeLimit<double>::Min());
        UNITTEST_TRUE(dx2._v.u_d < -10);
    }
};
