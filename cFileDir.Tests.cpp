...
namespace Gray {
struct UNITTEST_N(cFileDir) : public cUnitTest {
    ...
    void TestCreateFileInDir(cStringF sTestFile, FILESYS_t eFileSysType) {
        ...
        static const GChar_t* k_pszTimeChange = _GT("2008-07-09 13:47:10 Z");  // TIMEFORMAT_t::_DEFTZ
        cTimeInt tf;
        HRESULT hRes = tf.SetTimeStr(k_pszTimeChange);
        UNITTEST_TRUE(hRes == 0x15);
        cString sTimeChange2 = tf.GetTimeFormStr(TIMEFORMAT_t::_DEFTZ, TZ_UTC);
...
UNITTEST2_REGISTER(cFileDir, UNITTEST_LEVEL_t::_Core);
}  // namespace Gray
