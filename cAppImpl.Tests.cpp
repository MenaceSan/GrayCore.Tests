...
struct UNITTEST_N(cAppImpl) : public cUnitTest {
    ...
    UNITTEST_METHOD(cAppImpl) {
        ...
        if (pApp == nullptr) {
            ...
            sm_created = true;
            pApp->m_State.InitArgsWin(_FN("-t1=234234 -t2 123 -t2=234 -t1"));
        } else if (!sm_created) {
            // Add command calls to the end of the existing command line.
            cAppArgs& argSet = pApp->m_State.m_Args;
        ...
        cBitmask<> validPrev = pApp->m_ArgsValid;
        ...
        // Check m_ArgsValid
        const UINT validBits = pApp->m_ArgsValid;
