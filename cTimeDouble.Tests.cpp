...
struct UNITTEST_N(cTimeDouble) : public cUnitTest {
    ...
    UNITTEST_METHOD(cTimeDouble) {
        ...
        static const cTimeUnits k_aSysTimes[] = {            // DOW is tested elsewhere
        ...
        for (UINT i = 0; i < _countof(k_aSysTimes); i++) {
            UnitTest_1(k_aSysTimes[i]);
        }
        ...
        const double dDays = 1 * cTimeUnits::GetUnitDef(TIMEUNIT_t::_Day)._dUnitDays + 2 * cTimeUnits::GetUnitDef(TIMEUNIT_t::_Hour)._dUnitDays + 3 * cTimeUnits::GetUnitDef(TIMEUNIT_t::_Minute)._dUnitDays +
                       4 * cTimeUnits::GetUnitDef(TIMEUNIT_t::_Second)._dUnitDays + 5 * cTimeUnits::GetUnitDef(TIMEUNIT_t::_Millisecond)._dUnitDays + 6 * cTimeUnits::GetUnitDef(TIMEUNIT_t::_Microsecond)._dUnitDays;
