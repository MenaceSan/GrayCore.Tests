...
#if defined(GRAY_STATICLIB)  // make Gray* all static lib
...
#define UNITTEST2_REGISTER(n, lvl)                        \
    UNITTEST_REGISTER(n, lvl);                            \
    UNITTEST_EXT_EXP(n);                                  \
    TEST_CLASS(UNITTEST_MSN(n)) {                         \
     public:                                              \
        cUniquePtr<UNITTEST_N(n)> m_pTest;                \
        UNITTEST_MSN(n)() : m_pTest(new UNITTEST_N(n)) {} \
        TEST_METHOD(RunTest) { m_pTest->RunUnitTest(); }  \
    }
